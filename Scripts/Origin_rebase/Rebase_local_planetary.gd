...
func is_entered_local_space_planetary(zone):
	...
	if p.local_space_planetary.has_node("Scenes"):
		var scene_names = []

		for c in p.local_space_planetary.get_node("Scenes").get_children():
			scene_names.append(c)
			
		var message = "Local space '"+p.local_space_planetary.get_name() \
